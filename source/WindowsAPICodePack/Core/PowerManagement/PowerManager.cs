...
namespace Microsoft.WindowsAPICodePack.ApplicationServices
{
	...
	public static class PowerManager
	{
		...
		public static int BatteryLifePercent
		{
			get
			{
				// Because of the way this value is being calculated, it should not be limited to granularity as the data from the event (old
				// way) was.
				CoreHelpers.ThrowIfNotVista();
			...
			get
			{
				...
				lock (monitoronlock)
				{
					if (isMonitorOn == null)
					{
						...
						// Wait until Windows updates the power source (through RegisterPowerSettingNotification)
		...
		public static bool IsUpsPresent
		{
			get
			{
				...
				// Because the native method doesn't return the correct value for .UpsPresent, use .BatteriesAreShortTerm and
		...
		public static PowerPersonality PowerPersonality
		{
			get
			{
				PowerManagementNativeMethods.PowerGetActiveScheme(IntPtr.Zero, out var guid);
