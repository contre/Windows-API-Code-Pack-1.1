...
{
	// ContentProperty allows us to specify the text of the button as the child text of a button element in XAML, as well as explicitly set
	// with 'Text="<text>"' Note that this attribute is inherited, so it applies to command-links and radio buttons as well.
	/// <summary>
	...
	public abstract class TaskDialogButtonBase : TaskDialogControl
	{
		...
		// Note that we don't need to explicitly implement the add/remove delegate for the Click event; the hosting dialog only needs the
		// delegate information when the Click event is raised (indirectly) by NativeTaskDialog, so the latest delegate is always available.
