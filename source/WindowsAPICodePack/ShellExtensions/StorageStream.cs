...
{
	/// <summary>Options for commiting (flushing) an IStream storage stream</summary>
	[Flags]
	internal enum StorageStreamCommitOptions
	{
		/// <summary>Uses default options</summary>
		None = 0,

		/// <summary>Overwrite option</summary>
		Overwrite = 1,

		/// <summary>Only if current</summary>
		OnlyIfCurrent = 2,

		/// <summary>Commits to disk cache dangerously</summary>
	...
	/// <summary>A wrapper for the native IStream object.</summary>
	public class StorageStream : Stream, IDisposable
	{
		...
		/// <summary>Gets whether the stream can be read from.</summary>
		public override bool CanRead => _stream != null;

		/// <summary>Gets whether seeking is supported by the stream.</summary>
		public override bool CanSeek => _stream != null;

		/// <summary>Gets whether the stream can be written to. Always false.</summary>
		public override bool CanWrite => _stream != null && !_isReadOnly;

		/// <summary>Gets the length of the IStream</summary>
		public override long Length
		{
			get
			{
				...
				_stream.Stat(out var stats, STATFLAG_NONAME);
				return stats.cbSize;
			}
		}

		/// <summary>Gets or sets the current position within the underlying IStream.</summary>
		...
		/// <summary>Commits data to be written to the stream if it is being cached.</summary>
		public override void Flush() => _stream.Commit((int)StorageStreamCommitOptions.None);

		/// <summary>Reads a buffer worth of bytes from the stream.</summary>
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			if (count > 0)
			{
				var ptr = Marshal.AllocCoTaskMem(sizeof(ulong));
				try
				{
					if (offset == 0)
					{
		...
		/// <summary>Reads a single byte from the stream, moving the current position ahead by 1.</summary>
		...
		{
			ThrowIfDisposed();
			var ptr = Marshal.AllocCoTaskMem(sizeof(long));
			try
			{
		...
		/// <summary>Writes a buffer to the stream if able to do so.</summary>
		...
		{
			if (_stream == null) throw new ObjectDisposedException(GetType().Name);
		}
	}
}